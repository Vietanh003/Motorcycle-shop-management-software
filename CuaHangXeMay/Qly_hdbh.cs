...
namespace CuaHangXeMay
{
    public partial class Qly_hdbh : Form
    {
      ...
      private void btn_in_Click(object sender, EventArgs e)
      {
          ...
          string sql = "select H.MaHoaDon,H.NgayLap,H.TongTien,C.DonGia,C.GiamGia,C.ThanhTien,C.SoKhungXe,V.TenNhanVien,K.TenKhachHang,Xe.MaLoaiXeGanMay,T.TenHinhThuc from ChiTietHoaDon as C inner join HoaDon as H on C.MaHoaDon=H.MaHoaDon inner join KhachHang as K on K.MaKhachHang=H.MaKhachHang inner join NhanVienBanHang as N on N.MaNhanVien=H.MaNhanVien inner join NhanVien as V on V.MaNhanVien=N.MaNhanVien inner join Xe on Xe.SoKhung=C.SoKhungXe inner join LoaiXeGanMay as L on L.MaLoaiXeGanMay=Xe.MaLoaiXeGanMay inner join HinhThucThanhToan as T on T.MaHinhThuc=H.MaHinhThuc Where H.MaHoaDon='"+cbo_mahd.SelectedValue.ToString()+"'";
          DataTable dt_cthdtheomahd = db.getDatatable(sql);
          CTRP_HoaDon rphd = new CTRP_HoaDon();
          rphd.SetDataSource(dt_cthdtheomahd);
            Report f = new Report();
            f.DisplayReport(rphd);
            f.ShowDialog();
