...
namespace CuaHangXeMay
{
    public partial class Qly_PhieuNhap : Form
    {
        ...
        {
            string sql = "select H.MaPhieuNhap,H.NgayLap,H.TongTien,C.DonGia,C.SoKhungXe,V.TenNhanVien,N.Ten,Xe.MaLoaiXeGanMay ,C.SoMay from ChiTietPhieuNhap as C inner join PhieuNhap as H on C.MaPhieuNhap=H.MaPhieuNhap inner join NhaCungCap as N on N.MaNhaCungCap=H.MaNhaCungCap inner join QuanLy as Q on Q.MaNhanVien=H.MaNhanVien inner join NhanVien as V on V.MaNhanVien=Q.MaNhanVien inner join Xe on Xe.SoKhung=C.SoKhungXe inner join LoaiXeGanMay as L on L.MaLoaiXeGanMay=Xe.MaLoaiXeGanMay Where H.MaPhieuNhap='"+cbo_mapn.SelectedValue.ToString()+"'";
            DataTable dt_ctpnthepmapn = db.getDatatable(sql);
            CTRP_PhieuNhap rppn = new CTRP_PhieuNhap();
            rppn.SetDataSource(dt_ctpnthepmapn);
            Report f = new Report();
            f.DisplayReport(rppn);
            f.ShowDialog();
        }

        private void cbo_maloaixe_SelectedIndexChanged(object sender, EventArgs e)
        {
